...
class Marks extends RestController {
    ...
    public function __construct()
    {
		...
		$this->load->model('marks/Marks_model', 'model');
    }
    

    public function mark_get()
    {
        $this->response($this->model->get_marks());
    }
    ...
    public function mark_post()
    {
        ...
        if($validate['status']) {
            $this->response( $this->model->set_mark($this->post()) );
        } else {
            $this->response($validate);
        }
    }

    public function mark_put()
    {
        // validación de los campos
        $validate = $this->_validate_fields('put', $this->put());
        if($validate['status']) {
            $this->response( $this->model->update_mark($this->put()) );
        } else {
            $this->response($validate);
        }
    }
    ...
    public function mark_delete()
    {
        ...
        if($validate['status']) {
            $this->response($this->model->delete_mark($id));
        } else {
            $this->response($validate);
        }
    }
